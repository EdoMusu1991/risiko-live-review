...
import { PannelloReplay } from "@/componenti/PannelloReplay";
    ...
    <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Replay scrubabile: visibile se la ricostruzione ha prodotto
              uno stato_finale (= la partita è almeno parzialmente
              ricostruita e il bundle è esportabile). Il sync replay→video
              è cablato qui sotto via onCursorChange. Il sync inverso
              (video drives replay) rimane per scope C, dopo i primi test
              su una partita reale. */}
          {statoFinale && "stato_finale" in statoFinale ? (
            <section>
              <h3 className="etichetta mb-3">Replay scrubabile</h3>
              <PannelloReplay
                partitaId={p.id}
                territorioSelezionato={territorioSelezionato}
                onClickTerritorio={onClickTerritorio}
                onCursorChange={(tsEvento) => {
                  // Replay → video: sposta il PlayerVideo sull'offset
                  // corrispondente al timestamp dell'evento corrente.
                  // Stesso pattern usato da PannelloProposteAggregazione.
                  if (videoCorrente && tsEvento) {
                    const inizioVideo = new Date(
                      videoCorrente.ts_inizio,
                    ).getTime();
                    const tsTarget = new Date(tsEvento).getTime();
                    const offsetSec = (tsTarget - inizioVideo) / 1000;
                    if (offsetSec >= 0) saltaA(offsetSec);
                  }
                }}
