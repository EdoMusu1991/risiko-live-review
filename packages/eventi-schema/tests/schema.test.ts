...
// === Regression test: fixture reale del backend RL ===

describe("fixture reale bundle-replay-esempio.json", () => {
  it("è parsabile come BundleReplay senza errori", async () => {
    ...
    const fixturePath = path.join(
      ...
      "..",
      "fixtures",
      "bundle-replay-esempio.json",
    );
    const raw = JSON.parse(fs.readFileSync(fixturePath, "utf8"));

    const bundle = parsaBundleReplay(raw);
    expect(bundle.schema_version).toBe("1.0");
    expect(bundle.giocatori.length).toBeGreaterThanOrEqual(2);
    expect(bundle.eventi.length).toBeGreaterThan(0);

    // Verifica che ogni evento abbia tipo riconosciuto e narrowing funzioni
    let nAttacchi = 0;
    let nConquiste = 0;
    for (const ev of bundle.eventi) {
      if (ev.tipo === "attacco_risolto") {
        nAttacchi++;
        expect(ev.dati.dadi_attaccante.length).toBeGreaterThanOrEqual(1);
        expect(ev.dati.dadi_attaccante.length).toBeLessThanOrEqual(3);
      }
      if (ev.tipo === "territorio_conquistato") {
        nConquiste++;
      }
    }
    // Dovrebbe esserci almeno qualche attacco nella partita simulata
    expect(nAttacchi).toBeGreaterThan(0);
    // Le conquiste possono essere 0 se l'RNG ha dato dadi sfavorevoli
    expect(nConquiste).toBeGreaterThanOrEqual(0);
  });
});
