...
```bash
# 1. Build del pacchetto (richiesto prima del frontend)
cd packages/eventi-schema
npm install
npm run build   # produce dist/

# 2. Frontend (importa via file: dep)
cd ../../frontend
npm install
npm run dev
...
**Nota**: lo script `prepare` è stato rimosso intenzionalmente. Il
pacchetto va buildato manualmente prima del frontend, non automaticamente
da `npm install`. Motivo: il frontend installa il pacchetto come `file:`
dependency e npm chiamerebbe `prepare` senza che le `devDependencies` del
pacchetto siano disponibili (zod, typescript). Il workflow esplicito è
più predicibile e fallisce in modo chiaro.
